...
main:	addi $sp,$zero,0x800 		# $sp=0x800
	...
	addi $t0,$zero,0x3E8		
	sw   $t0,0x824			# BTCCR0=0x3E8=1000, for MCLK=50MHz we get PWMout=50kHz
	srl  $t0,$t0,2		
...
KEY1_ISR:	addi $t0,$zero,0xD5  
		...
		sw   $t0,0x804 		# write DIVIDEND low nibble to PORT_HEX0[7-0]
		srl  $t0,$t0,4	
		sw   $t0,0x805 		# write DIVIDEND high nibble to PORT_HEX1[7-0]
...
DIVIDER_ISR:	lw   $t4,0x834 		# read QUOTIENT value to $t4
		sw   $t4,0x808 		# write QUOTIENT low nibble to PORT_HEX2[7-0]
		srl  $t0,$t4,4
		sw   $t0,0x809 		# write QUOTIENT high nibble to PORT_HEX3[7-0]
		
		lw   $t4,0x838 		# read RESIDUE value to $t4
		sw   $t4,0x80C 		# write RESIDUE low nibble to PORT_HEX4[7-0]
		srl  $t0,$t4,4
		sw   $t0,0x80D 		# write RESIDUE high nibble to PORT_HEX5[7-0]	
